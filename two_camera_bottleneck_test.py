...
captureTest = "--capture-image-and-download --interval=2"
sudo = "sudo "
...
NUM_OF_PICS_PER_SHERD = 2.0 # Modify this for the number of pictures you want to take in a full cycle. The motor will stop this amount of times during the cycle to take pictures. Remember to add ".0" to make sure calculation will be precise. 
...
def captureImages(dir = '', name = ''):
    if (len(dir) == 0 or len(name) == 0):
        ################TESTING THIS LINE####################
        os.system(sudo+"gphoto2 "+captureTest) 
    else:
        ...
        for i in range(num_of_ports):
            ...
            if DEBUG_MODE:
                procs[i] = subprocess.Popen(['python', '/home/pi/Scripts/os_capture_test.py', \
                                             ports[i], dir+'/camera'+str(i)+'/'+name+str(i+1)+'.txt'], \
                                            stdout=subprocess.PIPE)
            else:
                print("apparently i am here god dammit")
                procs[i] = subprocess.Popen(['python', '/home/pi/Scripts/os_capture_test.py', \
                                             ports[i]], \
                                            stdout=subprocess.PIPE)
        '''if DEBUG_MODE:
...
def one_sherd_photo_cycle():
    ...
    for i in range(FULL_CYCLE_STEPS):
        if i >= next_angle_steps:
            ...
            ts = time.time()
            print("just called captureImagesfrom main program at: " + str(ts)[-5:]) #just return last 5 seconds of epoch
