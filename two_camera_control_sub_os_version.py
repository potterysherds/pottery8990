...
import json
import sys
from os_capture import capture_and_download # but not really needed here
# !!!you need to change the main script in that file for this one to work.!!!
...
def captureImages(dir_and_name = ""):
    if (len(dir_and_name) == 0):
        ...
        for i in range(len(ports)):
            ...
            if DEBUG_MODE:
                procs[i] = subprocess.Popen(['python', '/home/pi/Scripts/os_capture.py', \
                                             ports[i], dir_and_name+str(i+1), \
                                             dir_and_name+str(i+1)+'.txt'], \
        ...
        #for i in range(len(ports)):
        #    procs[i].wait()
        print('Photo group "'+dir_and_name+'" is finished')
