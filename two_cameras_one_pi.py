...
def captureImages(dir_and_name = ""):
    if (len(dir_and_name) == 0):
        ...
        procs = range(len(ports)) # Create a list of subprocesses
        #outps = range(len(ports)) # Stores subprocesses' outputs
        #erros = range(len(ports)) # Stores subprocesses' errors
        for i in range(len(ports)):
            #os.system(sudo+"gphoto2 "+captureCommand + port + "usb:" + ports[i] + ' --filename="'+dir_and_name+str(i+1)+suffix+'"')
            if DEBUG_MODE:
                fullCommandList = ['sudo', 'gphoto2', captureCommand, \
                               '--port', 'usb:'+ports[i], \
                               '--debug', '--debug-logfile', dir_and_name+str(i+1)+'.txt', \
                               '--filename', dir_and_name+str(i+1)+suffix]
                fullCommandStr = "sudo gphoto2 "+captureCommand + \
                                 port + "usb:" + ports[i] + \
                                 ' --debug --debug-logfile='+dir_and_name+str(i+1)+'.txt' + \
                                 ' --filename '+dir_and_name+str(i+1)+suffix
            else:
                ...
                fullCommandStr = "sudo gphoto2 "+captureCommand + \
                                 port + "usb:" + ports[i] + \
                                 ' --filename '+dir_and_name+str(i+1)+suffix# + " --interval -1"
            
            #print(fullCommandStr)
            procs[i] = subprocess.Popen(fullCommandList, stdout=subprocess.PIPE)
            #procs[i] = subprocess.Popen(fullCommandStr, stdout=subprocess.PIPE, shell=True)
        ...
        #killgp = subprocess.Popen(['killall', '-USR1', 'gphoto2'])
        if DEBUG_MODE:
            for i in range(len(ports)):
